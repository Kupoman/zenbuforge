...
class Session {
  ...
  _selectMaterial(id) {
    ...
    const material = this.project.materials.get(id);
    this.selections.textures = material.get('extensions').ZF_id.pbrMetallicRoughness.baseColorTexture.index ?? null;

    if (this.selections.textures) {
      const texture = this.project.textures.get(this.selections.textures);
      this.selections.samplers = texture.get('extensions').ZF_id.sampler;
    }
  }
