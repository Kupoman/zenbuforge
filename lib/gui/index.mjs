...
class Gui {
  ...
  _updateIO(results) {
    ...
    results.addScratchSessionUpdate({
      op: 'add',
      path: '/guiCaptureMouse',
      value: io.WantCaptureMouse && !this._isViewportActive,
    });

    results.addScratchSessionUpdate({
      op: 'add',
      path: '/guiCaptureKeyboard',
      value: io.WantCaptureKeyboard && !this._isViewportActive,
    });

    results.addScratchSessionUpdate({
      op: 'add',
      path: '/guiTextInput',
      value: io.WantTextInput,
    });
  }
  ...
  update(updates) {
    ...
    this.time += this.context.scratchSession.dt;

    ImGuiCanvas.NewFrame(this.time);
    ImGui.NewFrame();

    const model = {
      project: this.context.projectData,
      session: this.context.projectSession,
      clientSession: this.context.clientSession,
      projects: this.context.userSession.projects,
    };
    const { width, height } = this.context.scratchSession;
