...
describe('Translator', function () {
  ...
  describe('.applyJsonPatch', function () {
    ...
    it('should allow non-collection updates', function () {
      const translator = makeTranslator();
      translator.applyJsonPatch([{
        op: 'add',
        path: '/currentIndex',
      ...
      assert.strictEqual(translator.gltf.currentIndex, 1);
    });
    ...
    it('should destroy resources on collection update', function () {
      const translator = makeTranslator();
      translator.applyJsonPatch([{
        op: 'add',
        path: '/tests/new',
      ...
      assert.strictEqual(test.isDestroyed, true);
      const collectionSize = Object.keys(translator.collections['/tests'].resources).length;
      assert.strictEqual(collectionSize, 0);
    });

    it('should apply root update', function () {
      const translator = makeTranslator();
      translator.applyJsonPatch([{
        op: 'add',
        path: '/tests/new',
      ...
      assert.strictEqual(test.isDestroyed, true);
      const collectionSize = Object.keys(translator.collections['/tests'].resources).length;
      assert.strictEqual(collectionSize, 1);
    });
  });
});
