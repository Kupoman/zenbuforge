...
class Renderer {
  ...
  translate(key, data) {
    ...
    if (key === 'lights') {
      const _parser = {
        ...
        createUniqueName: () => GltfUtils.getId(localData),
        json: {
          extensions: {
            KHR_lights_punctual: {
              lights: [localData],
    ...
    if (key === 'nodes') {
      ...
      if (utils.isDefined(localData?.extensions?.KHR_lights_punctual?.light)) {
      ...
      if (utils.isDefined(localData.rotation)) {
        node.quaternion.set(...localData.rotation);
      }

      if (utils.isDefined(localData.scale)) {
  ...
  _rebuild(dirtySet) {
    ...
    GltfUtils.COLLECTION_PROPS.forEach((prop) => rebuildCollection(prop));
