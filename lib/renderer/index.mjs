...
class Renderer {
  ...
  reset() {
    ...
    if (this.canvas) {
      ...
      this.renderer.toneMappingExposure = 12.5 / (2 ** EXPOSURE);
      this.renderer.physicallyCorrectLights = true;
      this.renderTarget = new THREE.WebGLRenderTarget(1, 1, {
        type: THREE.FloatType,
        colorSpace: THREE.LinearSRGBColorSpace,
      });
      this.finalTarget = new THREE.WebGLRenderTarget(1, 1, {
      ...
      this.composer.writeBuffer = this.finalTarget;
      this.composer.renderTarget1 = this.finalTarget;
      this.composer.renderToScreen = false;
      this.composer.addPass(new RenderPass(null, null));
  ...
  translate(key, data) {
    ...
    if (key === 'scenes') {
      ...
      scene.userData.exposure = localData.extras.exposure;
