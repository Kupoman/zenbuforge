...
class Renderer {
  ...
  update(updates) {
    const updateSession = (scope) => {
      this[scope] = jsonpatch.default.applyPatch(
        this[scope],
        updates[scope],
        true,
        true,
    ...
    updateSession('projectSession');
    updateSession('clientSession');
    updateSession('userSession');

    this.translator.applyJsonPatch(updates.projectData);

    this.resize(this.projectSession.viewports[0]);
